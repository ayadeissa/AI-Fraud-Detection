...
def validate_batch_data(df):
    ...
    for col in NUMERICAL_COLS:
        ...
        # ==========================================
# 1. INDIVIDUAL FORM PREDICTION (نموذج الإدخال والتنبؤ)
# ==========================================
...
with st.form("customer_form"):
    ...
    with col1:
        loan_amount = st.number_input("Loan Amount", min_value=0.0, value=50000.0)
        credit_score = st.number_input("Credit Score", min_value=550, max_value=850, value=700)
        annual_income = st.number_input("Annual Income", min_value=0.0, value=100000.0)
        dti_ratio = st.number_input("DTI Ratio", min_value=0.0, max_value=1.0, value=0.30)
        employment_length_years = st.number_input("Employment Length (Years)", min_value=0, value=5)
        num_existing_loans = st.number_input("Existing Loans", min_value=0, value=2)

    with col2:
        account_age_months = st.number_input("Account Age (Months)", min_value=0, value=36)
        declared_purpose = st.selectbox("Declared Purpose", PURPOSES)
        primary_mcc_category = st.selectbox("Primary MCC Category", MCC)
        secondary_mcc_category = st.selectbox("Secondary MCC Category", MCC)
        mcc_mismatch_flag = st.selectbox("MCC Mismatch", [0, 1])
        days_to_first_tx = st.number_input("Days to First Transaction", min_value=0, value=10)

    with col3:
        pct_spent_48h = st.number_input("Spent in 48h", min_value=0.0, max_value=1.0, value=0.50)
        pct_spent_7d = st.number_input("Spent in 7 Days", min_value=0.0, max_value=1.0, value=0.70)
        cash_withdrawal_ratio = st.number_input("Cash Withdrawal Ratio", min_value=0.0, max_value=1.0, value=0.20)
        high_risk_spend_ratio = st.number_input("High Risk Spend Ratio", min_value=0.0, max_value=1.0, value=0.20)
        international_tx_ratio = st.number_input("International Transaction Ratio", min_value=0.0, max_value=1.0, value=0.10)
        nighttime_tx_ratio = st.number_input("Nighttime Transaction Ratio", min_value=0.0, max_value=1.0, value=0.10)
        num_unique_merchants = st.number_input("Unique Merchants", min_value=0, value=10)
        num_total_transactions = st.number_input("Total Transactions", min_value=0, value=20)
        avg_tx_amount = st.number_input("Average Transaction Amount", min_value=0.0, value=1000.0)
        max_single_tx_pct = st.number_input("Maximum Single Transaction %", min_value=0.0, max_value=1.0, value=0.50)

    submitted = st.form_submit_button("🚨 Predict Customer Risk", use_container_width=True)

# تنفيذ التنبؤ الفردي فور الضغط على الزر
if submitted:
    if artifact is not None:
        row = pd.DataFrame([{
        ...
        if errors:
            ...
            c1.metric("Fraud Probability", f"{probability:.1%}")
            c2.metric("Prediction", "MISUSE" if prediction == 1 else "LEGITIMATE")
            c3.metric("Risk Level", level)
            st.progress(min(float(probability), 1.0))

# ==========================================
# 2. BATCH DATA UPLOAD (رفع ملف CSV)
# ==========================================
st.divider()
uploaded_file = st.file_uploader("📁 Upload Dataset", type=["csv"], help="Upload a CSV file containing the model features.")
...
if uploaded_file is not None:
    ...
    if errors:
        ...
        if artifact is not None:
            ...
            df["risk_level"] = pd.cut(probabilities, bins=[-0.01, 0.50, 0.75, 1.0], labels=["LOW", "MEDIUM", "HIGH"])
